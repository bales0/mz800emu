...
static void test_decode_lep_runs_and_continuation(void)
{
    ...
    assert_pulse(stream, 5, 1);
    assert_pulse(stream, 130, 0);
    assert_pulse(stream, 2, 1);
    uint64_t units;
    int value;
...
{
    const uint8_t high_300[] = { 127, 0, 46 };
    const uint8_t low_300[] = {
        (uint8_t) (int8_t) -127, 0, (uint8_t) (int8_t) -46
    };
    const uint8_t high_254[] = { 127, 0 };
    const uint8_t high_255[] = { 127, 0, 1 };
...
{
    const uint8_t source[] = { 127, 0, 46, (uint8_t) (int8_t) -7, 9 };
    st_CMT_STREAM *stream = cmt_edge_stream_from_data(source, sizeof(source),
                                                      CMT_EDGE_LEP_RATE,
                                                      CMT_STREAM_POLARITY_NORMAL);
    TEST_ASSERT_NOT_NULL(stream);
    TEST_ASSERT_EQUAL_INT(EXIT_SUCCESS,
    ...
    fclose(fh);
    TEST_ASSERT_EQUAL_UINT(sizeof(source), saved_size);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(source, saved, sizeof(source));
    ...
    assert_pulse(loaded, 300, 1);
    assert_pulse(loaded, 7, 0);
    assert_pulse(loaded, 9, 1);
